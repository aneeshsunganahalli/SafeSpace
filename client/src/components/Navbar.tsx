            ...
            {isAuthenticated ? (
              <>
                ...
                {user?.currentStreak !== undefined && user.currentStreak > 0 && (
                  <Link
                    href="/progress" 
                    className="ml-3 flex items-center rounded-full bg-gradient-to-r from-orange-500 to-yellow-400 px-3 py-1.5 shadow-sm hover:from-orange-600 hover:to-yellow-500 transition-colors cursor-pointer"
                    title={`Longest streak: ${user.longestStreak || user.currentStreak} days. Click to view progress.`}
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-1.5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    ...
                    <span className="text-xs font-bold text-white">{user.currentStreak} day{user.currentStreak !== 1 ? 's' : ''}</span>
                  </Link>
                )}
                      ...
                      <div className="ml-3">
                        ...
                        {user?.currentStreak !== undefined && user.currentStreak > 0 && (
                          <div className="mt-2">
                            <Link
                              href="/progress"
                              className="inline-flex items-center rounded-full bg-gradient-to-r from-orange-500 to-yellow-400 px-3 py-1 shadow-sm hover:from-orange-600 hover:to-yellow-500 transition-colors"
                              onClick={() => setIsMobileMenuOpen(false)}
                            >
                              <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5 mr-1 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                                <path strokeLinecap="round" strokeLinejoin="round" d="M17.657 18.657A8 8 0 016.343 7.343S7 9 9 10c0-2 .5-5 2.986-7C14 5 16.09 5.777 17.656 7.343A7.975 7.975 0 0120 13a7.975 7.975 0 01-2.343 5.657z" />
                                <path strokeLinecap="round" strokeLinejoin="round" d="M9.879 16.121A3 3 0 1012.015 11.1c.5 2 .5 3.5-1.185 5.4" />
                              </svg>
                              <span className="text-xs font-bold text-white">{user.currentStreak} day streak</span>
                            </Link>
                          </div>
                        )}
                      </div>
